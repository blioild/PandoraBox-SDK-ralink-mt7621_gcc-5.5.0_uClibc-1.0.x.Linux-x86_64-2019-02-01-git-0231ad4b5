...
ifeq ($(DUMP),1)
  ...
  LINUX_VERMAGIC:=$(if $(LINUX_VERMAGIC),$(LINUX_VERMAGIC),406de485cfd16500cb4ff4515b1a81e6)
...
ifneq (,$(findstring uml,$(BOARD)))
  ...
  LINUX_KARCH := x86
else ifneq (,$(findstring $(ARCH) , lexra ))
  LINUX_KARCH := rlx
else
  LINUX_KARCH := $(ARCH)
endif
  ...
  $(if $(CONFIG_PACKAGE_kmod-$(1)),
      ...
      $(1)-disabled:
		@echo "WARNING: kmod-$(1) is not available in the kernel config - generating empty package" >&2
		 $$(error  kmod-$(1) is missing the kernel config)
      define Package/kmod-$(1)/install
		true
      endef
  )
