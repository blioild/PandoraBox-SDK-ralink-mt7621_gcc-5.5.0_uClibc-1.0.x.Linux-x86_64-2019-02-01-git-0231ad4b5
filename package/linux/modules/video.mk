...
define KernelPackage/fb-backlight
  SUBMENU:=$(VIDEO_MENU)
  TITLE:=Framebuffer TFT LCD support
  DEPENDS:=@DISPLAY_SUPPORT +kmod-fb +fb-base +kmod-pwm
  KCONFIG:= \
  ...
  FILES:= $(LINUX_DIR)/drivers/video/backlight/backlight.ko \
               $(LINUX_DIR)/drivers/video/backlight/lcd.ko \
               $(LINUX_DIR)/drivers/video/backlight/generic_bl.ko \
               $(LINUX_DIR)/drivers/video/backlight/pwm_bl.ko
               
  AUTOLOAD:=$(call AutoLoad,07,backlight generic_bl pwm_bl)
endef

define KernelPackage/fb-backlight/description
 Kernel support for framebuffers TFT LCD base
endef

$(eval $(call KernelPackage,fb-backlight))

define KernelPackage/fb-tft
  SUBMENU:=$(VIDEO_MENU)
  TITLE:=Framebuffer TFT LCD support
  DEPENDS:=@DISPLAY_SUPPORT +kmod-fb +kmod-pwm
  KCONFIG:= \
			CONFIG_FB_TFT=m  \
  ...
  AUTOLOAD:=$(call AutoLoad,06,fbtft)
endef
...
#FIXME:This Selection need to fix
define KernelPackage/st7735
  SUBMENU:=$(VIDEO_MENU)
  TITLE:=st7735 LCD support
  DEPENDS:=+kmod-fb  +kmod-spi-gpio
  KCONFIG:= \
			CONFIG_FB_TFT=m  \
  ...
  FILES:=   $(LINUX_DIR)/drivers/video/fbtft/fbtft.ko \
               $(LINUX_DIR)/drivers/video/fbtft/fbtft_device.ko \
               $(LINUX_DIR)/drivers/video/fbtft/flexfb.ko \
               $(LINUX_DIR)/drivers/video/fbtft/fb_st7735r.ko \

   AUTOLOAD:=$(call AutoLoad,80, fb_sys_fops fbtft  fb_st7735r  sysimgblt  syscopyarea sysfillrect backlight,1)
endef

define KernelPackage/st7735/description
   st7735 LCD support
endef

$(eval $(call KernelPackage,st7735))
#############################################################
